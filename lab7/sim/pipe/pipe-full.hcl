#/* $begin pipe-all-hcl */
# 522031910118 滕昊益
# 1. Add the iaddq instruction
# 2. Use load forwarding
...
bool D_stall = 
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 (E_dstM in {d_srcA,d_srcB} && ( E_dstM == d_srcB || !(D_icode in {IRMMOVQ,IPUSHQ})));
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVQ, IPOPQ } &&  (E_dstM in {d_srcA,d_srcB} && ( E_dstM == d_srcB || !(D_icode in {IRMMOVQ,IPUSHQ})))) &&
	  IRET in { D_icode, E_icode, M_icode };
